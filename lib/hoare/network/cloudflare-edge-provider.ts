...
/**
 * Governed Cloudflare mutation boundary.
 *
 * The provider intentionally accepts a DNS record id rather than discovering and
 * mutating a record in one opaque operation. The transaction authority is checked
 * immediately before and after the external mutation, and the tenant->zone binding
 * is explicit. No credential is persisted or serialized by this class.
 */
export class CloudflareEdgeProvider {
  ...
  async updateDnsRecord(request: CloudflareEdgeRequest): Promise<CloudflareEdgeResult> {
    ...
    if (request.authority.tenantId !== request.tenantId) {
      throw new Error("cloudflare_tenant_mismatch");
    }
    ...
    if (!response.ok) {
      throw new Error(`cloudflare_mutation_failed:${response.status}`);
    }
