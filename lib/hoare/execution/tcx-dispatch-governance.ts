...
export type TcxDispatchIntentStatus = "PENDING" | "PUBLISHED";
...
export class InMemoryTcxLeaseRepository implements TcxLeaseRepository {
  ...
  async get(leaseId: string): Promise<TcxLease | null> {
    const lease = this.leases.get(leaseId);
    return lease ? structuredClone(lease) : null;
  }

  async put(lease: TcxLease): Promise<TcxLease> {
    if (this.leases.has(lease.leaseId)) throw new Error("tcx_lease_already_exists");
  ...
  async revoke(leaseId: string, revokedAt = new Date().toISOString()): Promise<TcxLease> {
    ...
    if (!current) throw new Error("tcx_lease_not_found");
...
export class RedisTcxLeaseRepository implements TcxLeaseRepository {
  async get(leaseId: string): Promise<TcxLease | null> {
    const raw = await getRedis().get(safeKey(LEASE_KEY_PREFIX, leaseId));
    if (raw === null) return null;
    try {
      return JSON.parse(raw) as TcxLease;
    } catch (error) {
      throw new Error("tcx_lease_corrupt", { cause: error });
    }
  }

  async put(lease: TcxLease): Promise<TcxLease> {
    const result = await getRedis().set(
      safeKey(LEASE_KEY_PREFIX, lease.leaseId),
  ...
  async revoke(leaseId: string, revokedAt = new Date().toISOString()): Promise<TcxLease> {
    const key = safeKey(LEASE_KEY_PREFIX, leaseId);
    const client = getRedis();
    await client.watch(key);
    try {
      const raw = await client.get(key);
      if (raw === null) throw new Error("tcx_lease_not_found");
      const current = JSON.parse(raw) as TcxLease;
      const revoked = { ...current, revokedAt };
...
export class InMemoryTcxDispatchIntentRepository implements TcxDispatchIntentRepository {
  ...
  async get(dispatchKey: string): Promise<TcxDispatchIntent | null> {
    const intent = this.intents.get(dispatchKey);
    return intent ? structuredClone(intent) : null;
  }

  async create(intent: TcxDispatchIntent): Promise<TcxDispatchIntent> {
    const current = this.intents.get(intent.dispatchKey);
    if (current) return structuredClone(current);
    this.intents.set(intent.dispatchKey, structuredClone(intent));
    return structuredClone(intent);
  }

  async markPublished(dispatchKey: string, publishedAt = new Date().toISOString()): Promise<TcxDispatchIntent> {
    const current = this.intents.get(dispatchKey);
    if (!current) throw new Error("tcx_dispatch_intent_not_found");
    const published = { ...current, status: "PUBLISHED" as const, publishedAt };
    this.intents.set(dispatchKey, published);
...
export class RedisTcxDispatchIntentRepository implements TcxDispatchIntentRepository {
  async get(dispatchKey: string): Promise<TcxDispatchIntent | null> {
    const raw = await getRedis().get(safeKey(DISPATCH_INTENT_KEY_PREFIX, dispatchKey));
    if (raw === null) return null;
    try {
      return JSON.parse(raw) as TcxDispatchIntent;
    } catch (error) {
      throw new Error("tcx_dispatch_intent_corrupt", { cause: error });
    }
  }

  async create(intent: TcxDispatchIntent): Promise<TcxDispatchIntent> {
    const result = await getRedis().set(
      safeKey(DISPATCH_INTENT_KEY_PREFIX, intent.dispatchKey),
      JSON.stringify(intent),
      "NX",
    );
    if (result !== "OK") {
      const existing = await this.get(intent.dispatchKey);
  ...
  async markPublished(dispatchKey: string, publishedAt = new Date().toISOString()): Promise<TcxDispatchIntent> {
    const key = safeKey(DISPATCH_INTENT_KEY_PREFIX, dispatchKey);
    const client = getRedis();
    await client.watch(key);
    try {
      const raw = await client.get(key);
      ...
      const published = { ...current, status: "PUBLISHED" as const, publishedAt };
      const result = await client.multi().set(key, JSON.stringify(published)).exec();
...
export async function requireValidTcxLease(
  transaction: { transactionId: string; attemptId: string; leaseId?: string },
