...
function key(transactionId: string): string {
  if (!transactionId || transactionId.includes("\u0000")) {
    throw new Error("invalid_execution_transaction_id");
  }
  return `${KEY_PREFIX}:${transactionId}`;
}

function clone(transaction: ExecutionTransaction): ExecutionTransaction {
  return structuredClone(transaction);
}

/**
 * Durable transaction repository backed by the existing Redis singleton.
 * Redis is persistence/coordination storage here; it is not execution authority.
 */
export class RedisExecutionTransactionRepository implements ExecutionTransactionRepository {
  ...
  async get(transactionId: string): Promise<ExecutionTransaction | null> {
    const raw = await redis.get(key(transactionId));
    if (raw === null) return null;
    try {
      return JSON.parse(raw) as ExecutionTransaction;
    } catch (error) {
      throw new Error("execution_transaction_corrupt", { cause: error });
    }
  }
  ...
  async findByAttempt(
    ...
    if (transaction.attemptHistory?.some((attempt) => attempt.attemptId === attemptId)) {
      return transaction;
    }
    return null;
  }
    ...
    for (let attempt = 0; attempt < TRANSITION_RETRIES; attempt += 1) {
      ...
      try {
        ...
        const result = await client
          .multi()
          .set(transactionKey, JSON.stringify(updated))
          .exec();
