...
const transaction: TCXTransaction = { transactionId: "tx-1", attemptId: "attempt-1", tenantId: "tenant-1", expectedStateVersion: 1, stateVersion: 1, idempotencyKey: "idem-1", leaseId: "lease-1", state: "CREATED" };
...
async function gateFor(lease = true) {
  ...
  await transactions.create({ ...transaction, releaseDigest: "r", artifactDigest: "a", artifactRef: "ref", pasorPlanHash: "p", pasorUnitId: "u", workloadId: "w", agentId: "agent-1", nodeId: "node-1", packId: "pack-1", runtimeKind: "native", state: "CREATED", stateVersion: 1, expectedStateVersion: 1, createdAt: now.toISOString(), updatedAt: now.toISOString() } as never);
