...
const tenant: TenantResource = { id: "tenant-1", organizationId: "org-1", name: "Test Tenant", region: "us-central1", dataResidency: "US", isolation: "shared", status: "active" };
const application: ApplicationResource = { id: "app-1", tenantId: "tenant-1", name: "demo-app", vertical: "software", runtime: "container", desiredNodeId: "node-1", status: "running" };
const node: InfrastructureNode = { id: "node-1", name: "gcp-node", kind: "cloud", provider: "gcp", region: "us-central1", capabilities: ["cloud-run", "container"], capacity: { cpu: 4, memoryGb: 16, storageGb: 100 }, status: "online" };
const plan: BuilderPlan = {
  id: "plan-1",
  intent: { tenantId: "tenant-1", name: "demo", description: "deploy demo-app", resources: ["application"] },
  resources: [{ kind: "application", name: "demo-app", dependsOn: [] }],
