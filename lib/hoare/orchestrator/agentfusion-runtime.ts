...
export interface HoareAdmissionGate { admit(input: { tenantId: string; agentId: string; decision: OrchestratorDecision; context: AgentLoopState }): Promise<TCXAdmission>; }
...
export type TcxAuthorityIssuer = (transactionId: string) => Promise<GovernedExecutionAuthority>;
...
export class AgentFusionHoareRuntime implements HoareAgentRuntime {
  constructor(
    ...
    private readonly authorityIssuer?: TcxAuthorityIssuer,
  ) {}
  ...
  async execute(input: { decision: OrchestratorDecision; context: AgentLoopState }): Promise<{ success: boolean; detail: string }> {
    ...
    if (!admission.admitted || !admission.fenceValid) return { success: false, detail: `TCX admission denied: ${admission.reason ?? "invalid admission"}` };
    if (!this.authorityIssuer) return { success: false, detail: "TCX execution authority issuer is not configured" };
    const authority = await this.authorityIssuer(admission.transactionId);
    ...
    const result = await this.runtime.executeAgentGoverned(
      ...
      { transactionId: admission.transactionId, attemptId: admission.attemptId, fenceController: this.fenceController, authority },
    );
