...
export async function createTestTcxAuthority(overrides: Overrides = {}): Promise<GovernedExecutionAuthority> {
  const transaction: ExecutionTransaction = {
    ...
    releaseDigest: "release-test",
    artifactDigest: "artifact-test",
    artifactRef: "artifact-test",
    pasorPlanHash: "plan-test",
    pasorUnitId: "unit-test",
  ...
  const leases = { async get(): Promise<TcxLeaseRepository extends { get(...args: never[]): infer R } ? Awaited<R> : never> {
    return {
      leaseId: transaction.leaseId!,
      transactionId: transaction.transactionId,
      attemptId: transaction.attemptId,
      tenantId: transaction.tenantId,
      holderId: transaction.agentId,
      issuedAt: "2026-09-06T00:00:00.000Z",
      expiresAt: "2099-01-01T00:00:00.000Z",
    } as never;
  } } as unknown as TcxLeaseRepository;
