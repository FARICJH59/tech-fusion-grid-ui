...
const GCP_TEST_ENV = {
  GOOGLE_CLOUD_PROJECT_ID: "caramel-limiter-495010-b9",
  GOOGLE_CLOUD_REGION: "us-central1",
  GOOGLE_CLOUD_WIF_PROVIDER: "projects/123/locations/global/workloadIdentityPools/test/providers/test",
  GOOGLE_CLOUD_WIF_SERVICE_ACCOUNT: "test-hoare@example.invalid",
} as const;

function withTestWif<T>(fn: () => T): T {
  const names = Object.keys(GCP_TEST_ENV) as Array<keyof typeof GCP_TEST_ENV>;
  const saved = Object.fromEntries(names.map((name) => [name, process.env[name]]));
  for (const name of names) process.env[name] = GCP_TEST_ENV[name];
  try { return fn(); } finally { for (const name of names) { const value = saved[name]; if (value === undefined) delete process.env[name]; else process.env[name] = value; } }
}

test("phase7 cloud profile uses WIF and official GCP SDK contracts", () => withTestWif(() => {
  const profile = createGoogleCloudProfile();
  const wif = createWifConfig();
  assert.equal(profile.projectId, GCP_TEST_ENV.GOOGLE_CLOUD_PROJECT_ID);
...
test("phase7 cost engine emits optimization recommendations", () => {
  ...
  engine.ingest({ tenantId: "tenant-1", promptTokens: 100, completionTokens: 500, embeddingsTokens: 120_000, imageGenerations: 10, videoGenerations: 5, gpuSeconds: 25_000, cloudRunVcpuSeconds: 20_000, cloudRunMemoryGbSeconds: 20_000, estimatedCostUsd: 23.4 });
...
test("phase7 autonomous scaling transforms cloud run plan", () => {
  const plan = autonomousScalingEngine.applyRecommendations([{ tenantId: "tenant-1", category: "cloud-run-concurrency", priority: "high", message: "tune" }], { minInstances: 1, maxInstances: 3, concurrency: 20, cpu: "2", memory: "2Gi", region: "us-east1" });
  assert.ok(plan.concurrency >= 80); assert.ok(plan.maxInstances >= 3);
});

test("phase7 fleet manager supports multi-region failover", () => {
  const fleet = createDefaultFleetManager();
  const placement = fleet.placeWorkload({ tenantId: "tenant-1", requiredCapacity: 20, latencySensitive: true });
  assert.ok(placement.primary.length > 0); assert.ok(placement.failover.length > 0);
});

test("phase7 integration layer registers connector plugins", async () => {
  const layer = createDefaultIntegrationLayer(); const names = layer.list();
  assert.equal(names.length, ENTERPRISE_CONNECTORS.length);
  const result = await layer.run("GitHub", { tenantId: "tenant-1", actorId: "user-1", payload: { event: "sync" } });
...
test("phase7 messaging runtime enforces ACL checks and dead-letter route", () => {
  ...
  messaging.setTenantAcl("tenant-1", [{ tenantId: "tenant-1", topic: "tenant/1/events", access: "readwrite" }]);
