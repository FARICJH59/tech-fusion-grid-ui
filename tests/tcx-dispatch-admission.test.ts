...
test("TCX admission rejects an unclaimed dispatch intent", async () => {
  const ctx = await setup();
  const dispatchIntents = new InMemoryTcxDispatchIntentRepository();
  await dispatchIntents.create({ dispatchKey: ctx.dispatchKey, transactionId: ctx.transaction.transactionId, attemptId: ctx.transaction.attemptId, attemptNumber: ctx.transaction.attemptNumber, stateVersion: ctx.envelope.stateVersion, idempotencyKey: ctx.transaction.idempotencyKey, channelId: ctx.transaction.channelId, status: "PENDING", createdAt: NOW.toISOString() });
  await assert.rejects(admitTcxDispatch(ctx.envelope, { ...ctx, dispatchIntents }, NOW), /tcx_dispatch_intent_not_claimed/);
});

test("TCX admission rejects cross-attempt identity", async () => {
  const ctx = await setup();
  const envelope = { ...ctx.envelope, attemptId: "other-attempt" };
  await assert.rejects(admitTcxDispatch(envelope, ctx, NOW), /tcx_dispatch_attempt_id_mismatch/);
});
...
test("TCX admission rejects idempotency identity mismatch", async () => {
  ...
  await assert.rejects(admitTcxDispatch(envelope, ctx, NOW), /tcx_dispatch_idempotency_key_mismatch/);
});
...
test("TCX admission rejects stale envelope state version", async () => {
  ...
  await assert.rejects(admitTcxDispatch(envelope, ctx, NOW), /tcx_dispatch_intent_state_version_mismatch/);
});
...
test("TCX admission fences an intervening transaction mutation", async () => {
  ...
  await assert.rejects(admitTcxDispatch(ctx.envelope, ctx, NOW), /tcx_dispatch_state_version_mismatch/);
});
