...
export function createPasorPlan(inventory: ProjectInventory): PasorPlan {
  ...
  const add = (unit_id: string, command_id: string, parameters: Record<string, unknown>, dependencies: string[], energy_cost: number, quota_cost: number, optional = false) => {
    ...
    units.push({ ...base, ...(optional ? { optional: true } : {}), simulation_hash: hash({ phase: "simulation", ...base }), provenance_hash: hash({ phase: "provenance", tenant_id: inventory.tenant_id, project_id: inventory.project_id, inventory_hash: inventory.provenance_hash, ...base }) });
  };
  ...
  if (inventory.detected.has_aegisc) add("aegisc-verify", "aegisc.verify", { execution_contract: true, plan_integrity: true, fencing: true }, ["build"], 12, 3);
  if (inventory.detected.has_pasor) add("pasor-validate", "pasor.validate", {}, ["inventory"], 3, 1);

  const planCore: Pick<PasorPlan, "schema" | "project_id" | "tenant_id" | "execution_units"> = {
    schema: "hoare.pasor-plan/v1",
  ...
  return {
    ...planCore,
    totals: { energy_cost: units.reduce((n, u) => n + u.energy_cost, 0), quota_cost: units.reduce((n, u) => n + u.quota_cost, 0) },
