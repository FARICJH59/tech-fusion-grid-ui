...
impl<'a> Parser<'a> {
 ...
 fn body(&mut self)->Result<Vec<Stmt>,String>{self.expect(TokenKind::LBrace)?;let body=self.statements_until_rbrace()?;self.expect(TokenKind::RBrace)?;Ok(body)}
 fn statements_until_rbrace(&mut self)->Result<Vec<Stmt>,String>{let mut body=Vec::new();while self.cur().kind!=TokenKind::RBrace{if let TokenKind::Word(w)=&self.cur().kind{if w=="if"{body.push(self.if_stmt()?);continue;}}let name=self.ident()?;self.expect(TokenKind::Dot)?;let op=self.ident()?;self.expect(TokenKind::LParen)?;self.skip_parens()?;self.expect(TokenKind::Semicolon)?;match op.as_str(){"commit"=>body.push(Stmt::Commit(name)),"abort"=>body.push(Stmt::Abort(name)),_=>body.push(Stmt::Use(name))}}Ok(body)}
 fn if_stmt(&mut self)->Result<Stmt,String>{self.word("if")?;let condition=self.ident()?;let then_body=self.body()?;self.word("else")?;let else_body=self.body()?;Ok(Stmt::If{condition,then_body,else_body})}
