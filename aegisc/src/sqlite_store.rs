...
impl SqliteHandoffStore {
    pub async fn claim(
        ...
        intent: &str,
        worker_id: &str,
    ) -> Result<Option<Lease<String>>, sqlx::Error> {
        self.claim_with_timeout(intent, worker_id, 16).await
    }
    ...
    pub async fn claim_with_timeout(
        ...
        let timeout_modifier = format!("+{timeout_ms} milliseconds");
        ...
        Ok(row.map(|r| {
            Lease::new(
                r.get::<String, _>("handoff_id"),
                FencingToken(r.get::<i64, _>("fencing_token")),
                worker_id.to_string(),
    ...
    pub async fn commit(
        ...
        .bind(result_json)
        .bind(&lease.handoff_id)
        .bind(lease.fencing_token.0)
        .bind(&lease.worker_id)
        .execute(&self.pool)
        .await
        .map_err(|_| RuntimeError::StaleFence)?;
